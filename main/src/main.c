...
main(int argc, char **argv)
{
	...
	SDL_RWops *f = SDL_RWFromFile(rom_filename, "r");
	if (!f) {
		printf("Cannot open %s!\n", rom_path);
		exit(1);
	}
	int rom_size = f->read(f, ROM, 1, ROM_SIZE);
	(void)rom_size;
	f->close(f);

#ifndef VERA_V0_8
	f = SDL_RWFromFile(char_filename, "r");
	if (!f) {
		printf("Cannot open %s!\n", char_path);
		exit(1);
	}
	uint8_t chargen[4096];
	int chargen_size = f->read(f, chargen, 1, sizeof(chargen));
	(void)chargen_size;
	f->close(f);
		...
		if (new_frame) {
			...
			if (log_speed) {
				...
				if ((int)frames_behind > 0) {
					SDL_Log("Rendering is behind %d frames.\n", -(int)frames_behind);
				} else {
				}
			}
		}
